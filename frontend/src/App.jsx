...
const skillGroups = [
  { title: 'Full-stack web development', skills: ['React', 'JavaScript', 'Node.js', 'Express', 'Python', 'FastAPI'] },
  { title: 'Databases & backend', skills: ['MongoDB', 'Mongoose', 'REST APIs', 'Authentication'] },
  { title: 'AI development', skills: ['OpenAI API', 'LLM applications', 'CrewAI', 'AI agents'] },
  { title: 'Tools & integrations', skills: ['Git', 'GitHub', 'Stripe', 'Resend', 'Cloudinary', 'Redis', 'Render'] },
  { title: 'Currently exploring', skills: ['TypeScript', 'NestJS', 'PostgreSQL', 'PostGIS'], exploring: true },
]

const copy = {
  fi: {
    nav: ['Minusta', 'Osaaminen', 'Projektit', 'CV', 'Ota yhteyttä'],
    ...
    skills: 'Tech universe', projects: 'Valittuja projekteja', projectLabel: 'FEATURED CASE / FULL-STACK E-COMMERCE', projectTitle: 'Dreamland v2', projectIntro: 'Korubrändille rakennettu täyden palvelun verkkokauppa — tuotteen löytämisestä maksun vahvistukseen.', projectBody: 'Rakensin kirjautumisen, kategoriat, ostoskorin, alennuskoodit, Stripe-maksuvirran, tilausviestit sekä roolipohjaisen hallintanäkymän tuotteiden ja myynnin hallintaan.', liveDemo: 'Avaa live-demo', viewCode: 'Katso koodi',
    contact: 'Ota yhteyttä', contactText: 'Keskustellaan mahdollisuuksista, projekteista tai hyvästä pizzasta.',
  ...
  en: {
    nav: ['About', 'Skills', 'Projects', 'CV', 'Contact'], eyebrow: 'OPEN TO WORK · AI & FULL-STACK',
    ...
    skills: 'Tech universe', projects: 'Selected work', projectLabel: 'FEATURED CASE / FULL-STACK E-COMMERCE', projectTitle: 'Dreamland v2', projectIntro: 'A full-stack jewellery store for a fictional brand — from product discovery to payment confirmation.', projectBody: 'Built customer authentication, category browsing, cart flows, discount coupons, Stripe Checkout, transactional emails and a role-protected admin dashboard for products and sales analytics.', liveDemo: 'Open live demo', viewCode: 'View code',
    contact: 'Get in touch', contactText: 'Let’s talk opportunities, projects or great pizza.',
...
function App() {
  ...
  return <main>
    ...
    <nav><a className="logo" href="#top">TS<span>✦</span></a><div className="nav-links">{t.nav.map((item, i) => <a href={['#about','#skills','#projects','/cv','#contact'][i]} key={item}>{item}</a>)}</div><div className="language"><button className={language === 'fi' ? 'active' : ''} onClick={() => changeLanguage('fi')}>FI</button><button className={language === 'en' ? 'active' : ''} onClick={() => changeLanguage('en')}>EN</button></div></nav>
    <section className="hero" id="top"><div className="hero-copy"><p className="eyebrow">{t.eyebrow}</p><h1>{t.title}</h1><p className="intro">{t.intro}</p><div className="hero-actions"><a className="button primary" href="#contact">{t.contact} <span>↗</span></a><a className="button ghost" href="https://github.com/TiinaTuuliaS" target="_blank" rel="noreferrer">GitHub ↗</a><a className="button ghost" href="https://www.linkedin.com/in/tiina-siremaa-7589a61b5/" target="_blank" rel="noreferrer">LinkedIn ↗</a></div><div className="mini-stats"><span>✦ Tampere, Finland</span><span>◌ Available for opportunities</span></div></div><div className="identity-card"><img className="portrait-photo" src="/tiina-portrait.png" alt="Tiina Siremaa"/><div className="card-glow"/><p>TIINA SIREMAA</p><div className="card-bottom"><span>DEVELOPER<br/>+ CREATIVE</span><span>2026<br/>PORTFOLIO</span></div></div></section>
    <section className="content" id="about"><div className="chat-panel"><div className="panel-head"><div><p className="eyebrow small">AI POWERED</p><h2>{t.chatTitle}</h2></div><span className="online"><i/> ONLINE</span></div><p className="subtitle">{t.chatSubtitle}</p><div className="messages">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`message ${message.role}`}><span>{message.role === 'assistant' ? 'TS' : 'YOU'}</span><p>{message.content}</p></div>)}{loading && <div className="typing"><i/><i/><i/></div>}</div><div className="quick-list">{t.quick.map((item) => <button key={item} onClick={() => sendMessage(item)}>{item}<span>↗</span></button>)}</div><form className="chat-input" onSubmit={(event) => { event.preventDefault(); sendMessage() }}><input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder={t.placeholder} maxLength="4000"/><button disabled={loading}>{t.send} <span>↑</span></button></form></div>
      <aside><section className="about-card"><p className="eyebrow small">01 / ABOUT</p><h2>Human-centred<br/>by nature.</h2><p>Developer, team player, cycling enthusiast and lifelong learner.</p><a href="https://github.com/TiinaTuuliaS" target="_blank" rel="noreferrer">GitHub profile ↗</a></section><section className="skills-card" id="skills"><p className="eyebrow small">02 / {t.skills}</p><div className="skills-groups">{skillGroups.map((group) => <div className={`skill-group${group.exploring ? ' exploring' : ''}`} key={group.title}><h3>{group.title}</h3><div className="skill-cloud">{group.skills.map((skill) => <span key={skill}>{skill}</span>)}</div></div>)}</div></section></aside></section>
    <section className="projects" id="projects"><div className="projects-heading"><p className="eyebrow">03 / {t.projects}</p><h2>Built to be<br /><em>used.</em></h2></div><article className="project-case"><div className="project-visual" aria-label="Dreamland v2 visual identity"><div className="project-star">✦</div><p>DREAMLAND</p><span>SOFT FUTURE<br />JEWELRY</span><div className="project-orb project-orb-one" /><div className="project-orb project-orb-two" /><div className="project-ring" /></div><div className="project-copy"><p className="eyebrow small">{t.projectLabel}</p><h3>{t.projectTitle}</h3><p className="project-intro">{t.projectIntro}</p><p className="project-body">{t.projectBody}</p><div className="project-stack">{['React', 'Node.js', 'Express', 'MongoDB', 'Stripe', 'Cloudinary'].map((skill) => <span key={skill}>{skill}</span>)}</div><div className="project-actions"><a className="button primary" href="https://verkkokauppa-projekti.onrender.com/" target="_blank" rel="noreferrer">{t.liveDemo} ↗</a><a className="button ghost" href="https://github.com/TiinaTuuliaS/dreamland-v2" target="_blank" rel="noreferrer">{t.viewCode} ↗</a></div></div></article></section>
    <section className="contact" id="contact"><div><p className="eyebrow">04 / CONTACT</p><h2>{t.contact}</h2><p>{t.contactText}</p></div><form onSubmit={sendContact}><input required value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder={t.name}/><input required type="email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} placeholder={t.email}/><textarea required value={form.message} onChange={(e) => setForm({...form, message: e.target.value})} placeholder={t.note}/><button className="button primary" disabled={contactState === 'sending'}>{contactState === 'sending' ? t.sending : `${t.contact} ↗`}</button>{contactState === 'sent' && <p className="form-status success">{t.sent}</p>}{contactState === 'error' && <p className="form-status error">{t.error}</p>}</form></section>
